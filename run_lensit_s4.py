#!/usr/bin/env python3
"""
run_lensit_s4.py  — LensIt MAP reconstruction, S4-like case.

  1.0 µK-arcmin, 1.0' beam, Lmax=12000, 512×512 patch (θpix≈0.744')

Output (JLD2-compatible HDF5, readable by JLD2.jl):
  results/lensit/s4/WL_lensit_qe_s4.jld2          — QE transfer functions
  results/lensit/s4/phi_maps_lensit_qe_s4.jld2     — per-sim ϕ_true, ϕ_qe_raw
  results/lensit/s4/WL_lensit_map_s4.jld2          — MAP transfer functions
  results/lensit/s4/phi_maps_lensit_map_s4.jld2    — per-sim ϕ_true, ϕ_mj

Usage:
  conda run -n lensit python run_lensit_s4.py
"""
...
CG_maxiter   = 500
BFGS_MEMORY  = 20
GRAD_CONV    = 0.01           # stop early when grad_ratio < 1%

nTpix = NlevT / θpix_arcmin

# Paths
out_dir      = 'results/lensit/s4'
lib_dir_iso  = 'TEMP/lensit_s4_isocov'
lib_dir_sims = 'TEMP/lensit_s4_sims'
iter_base    = 'TEMP/lensit_s4_iter'
qe_wl_file   = f'{out_dir}/WL_lensit_qe_s4.jld2'
qe_phi_file  = f'{out_dir}/phi_maps_lensit_qe_s4.jld2'
wl_file      = f'{out_dir}/WL_lensit_map_s4.jld2'
phi_file     = f'{out_dir}/phi_maps_lensit_map_s4.jld2'
ckpt_file    = f'{out_dir}/checkpoint_s4.npz'

os.makedirs(out_dir, exist_ok=True)
os.makedirs(iter_base, exist_ok=True)

print(f"\n{'='*60}")
print(f"  S4 LensIt MAP")
...
H0_pass = np.where((cpp > 0) & (_N0 > 0),
                   _N0 * cpp / np.maximum(_N0 + cpp, 1e-300), 0.)
print(f"  N0_len max={N0_len.max():.4e}  H0_pass max={H0_pass.max():.4e}")
...
# Iterator filter + CG chain (S4: standard mgchain, κ~26 converges in ~40 CG steps)
filt_ideal  = ffs_ninv_filt_ideal.ffs_ninv_filt(
    lib_alm, lib_alm, cls_unl, cl_transf, NlevT, nlev_p=1e6)
chain_descr = chain_samples.get_defaultmgchain(
    filt_ideal.lib_skyalm.ellmax, filt_ideal.lib_skyalm.lsides,
    filt_ideal.lib_datalm.shape, tol=CG_tol, iter_max=CG_maxiter)
opfilt_cinv._type = 'T'
...
for idx in range(nsims_done, N_sims):
    ...
    try:
        ...
        plm_qe = 0.5 * isocov.get_qlms('T', iblm, lib_qlm, use_cls_len=False)[0]
        lib_qlm.almxfl(plm_qe, N0_unl[:lib_qlm.ellmax + 1], inplace=True)
        phi_qe = lib_qlm.alm2map(plm_qe)
        # MAP warm start: QE output (no explicit Wiener filter for S4)
        plm_wf = plm_qe.copy()
        ...
        itlib = ffs_iterator_pertMF(
            ...
            verbose=False, NR_method=BFGS_MEMORY, no_deglensing=False)
        ...
        iter_times  = []
        t_last = t_sim_start
        print(f"  it=  0  (QE warm start)", flush=True)
